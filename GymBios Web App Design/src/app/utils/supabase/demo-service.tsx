...
class DemoService {
  ...
  isDemoMode(): boolean {
    ...
    return !!(
      ...
      currentUser?.email === 'admin@gymbios.com' ||
  ...
  // Check if provided credentials are demo credentials
  isDemoCredentials(email: string, password: string): boolean {
    const demo = this.getDemoCredentials();
    return email === demo.email && (password === demo.password || password === demo.altPassword);
  }
