...
export function Referrals() {
  ...
  const [referralFormData, setReferralFormData] = useState<any>(null);
  ...
  const handlePhotoUpload = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setReferralPhoto(reader.result as string);
        setPhotoFileName(file.name);
        toast.success('Photo Uploaded!', {
          description: 'Referral photo has been uploaded successfully.',
        });
      };
  ...
  const handleRemovePhoto = useCallback(() => {
    setReferralPhoto(null);
    setPhotoFileName('');
    if (fileInputRef.current) {
      fileInputRef.current.value = '';
    }
    toast.info('Photo Removed', {
      description: 'Referral photo has been removed.',
    });
  }, []);

  // Handle save referral - opens grant access dialog
  const handleSaveReferral = useCallback(() => {
    // Validate required fields
    if (!referralPhoto) {
      toast.error('Photo Required', {
        description: 'Please capture or upload a photo of the referral person for access control.',
      });
      return;
    }

    // Store form data (in real app, you'd collect from form inputs)
    const formData = {
      visitDate: visitDate,
      photo: referralPhoto,
  ...
  }, [referralPhoto, visitDate, photoFileName]);

  // Handle grant access confirmation
  const handleGrantAccess = useCallback(() => {
    // Simulate sending data to face access control device
    toast.loading('Connecting to Face Access Control...', {
      description: 'Please wait while we configure access...',
    });

    // Simulate API call to face access control device
    setTimeout(() => {
      toast.dismiss();
      toast.success('Access Granted Successfully!', {
        description: `Referral person has been granted gym access for ${new Date(visitDate).toLocaleDateString()}. Face recognition configured.`,
        duration: 5000,
      });
  ...
  return (
    <div className="p-6 space-y-6">
          ...
          <div className="space-y-4 mt-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1 mb-2">
                Capture or upload a photo for gym access verification
              </p>
              ...
              {!referralPhoto ? (
                ...
                <div className="space-y-2">
                  <div className="relative rounded-lg border-2 border-dashed border-[#2B7A78] p-2 bg-[#2B7A78]/5">
                    <img 
                      src={referralPhoto} 
                      alt="Referral person" 
                  ...
                  <div className="flex items-center justify-between text-sm text-muted-foreground">
                    <div className="flex items-center gap-2">
                      <Image className="h-4 w-4 text-[#2B7A78]" />
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1 mb-2">
                Date when the referral person will visit the gym
              </p>
              <Input 
                type="date" 
                className="mt-1" 
      ...
      <Dialog open={showGrantAccessDialog} onOpenChange={setShowGrantAccessDialog}>
        <DialogContent className="max-w-md rounded-2xl">
          <DialogHeader>
            <DialogTitle className="text-[#2B7A78] flex items-center gap-2">
              <CheckCircle className="h-5 w-5" />
              Grant Gym Access
            </DialogTitle>
            <DialogDescription>
              Confirm to grant access to the face access control device
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-4 mt-4">
            <Alert className="border-[#2B7A78] bg-[#2B7A78]/5">
              <Info className="h-4 w-4 text-[#2B7A78]" />
              <AlertDescription className="text-sm">
                This will configure the face access control system to grant gym entry for the referral person on the specified visit date.
              </AlertDescription>
            </Alert>
            ...
            <div className="bg-gray-50 rounded-lg p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold">{new Date(visitDate).toLocaleDateString('en-US', { 
                  weekday: 'short', 
                  year: 'numeric', 
                  month: 'short', 
                  day: 'numeric' 
              ...
              {referralPhoto && (
                <div>
                  <span className="text-sm text-muted-foreground mb-2 block">Photo Preview:</span>
                  <div className="relative rounded-lg border-2 border-[#2B7A78] p-2 bg-white">
                    <img 
                      src={referralPhoto} 
                      alt="Referral person" 
            ...
            {/* Access Control Info */}
            <div className="space-y-2">
              <h4 className="font-semibold text-sm">Access Control Device</h4>
              <ul className="text-sm text-muted-foreground space-y-1 pl-4">
                <li className="flex items-start gap-2">
                  <CheckCircle className="h-4 w-4 text-green-600 mt-0.5 flex-shrink-0" />
                  <span>Face recognition data will be uploaded</span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle className="h-4 w-4 text-green-600 mt-0.5 flex-shrink-0" />
                  <span>Access granted for visit date: {new Date(visitDate).toLocaleDateString()}</span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle className="h-4 w-4 text-green-600 mt-0.5 flex-shrink-0" />
                  <span>Real-time sync with access control system</span>
                </li>
              </ul>
            </div>
          </div>
