              ...
              {filteredProducts.map((product) => (
                ...
                >
                  <CardContent className="p-3">
                    <div className="aspect-square bg-gradient-to-br from-[#F9FAFB] to-white rounded-lg mb-2 flex items-center justify-center border-2 border-gray-100 max-h-24">
                      <Package className="h-8 w-8 text-[#2B7A78] opacity-30" />
                    </div>
                    <h3 className="text-xs mb-1.5 line-clamp-2 text-[#1E293B]">{product.name}</h3>
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-semibold text-[#2B7A78]">{formatCurrency(product.price)}</p>
                      <Badge 
                        variant={product.stock > 10 ? 'default' : 'destructive'}
                        className={product.stock > 10 ? 'bg-[#2B7A78] text-xs px-1.5 py-0' : 'text-xs px-1.5 py-0'}
                      >
                        {product.stock}
                      </Badge>
                    </div>
      ...
      <Dialog open={showPaymentDialog} onOpenChange={setShowPaymentDialog}>
        <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
          <DialogHeader className="space-y-3 pb-4 border-b">
            <div className="flex items-center justify-between">
              <DialogTitle className="text-2xl font-bold flex items-center space-x-3">
                <div className="flex items-center justify-center w-10 h-10 bg-gradient-to-r from-[#2B7A78] to-[#17a2b8] rounded-full">
            ...
            <DialogDescription>
              Choose your preferred payment method to complete this sale
            </DialogDescription>
            ...
            {currentInvoice.items.length > 0 && (
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-semibold text-blue-900">Sale Transaction</h3>
                    <p className="text-sm text-blue-700">
                      {selectedCustomerData?.name || 'Walk-in Customer'} • {currentInvoice.items.length} items
                    </p>
                  </div>
                  <div className="text-right">
                    ...
                    {currentInvoice.totalDiscount > 0 && (
                      <div className="text-xs text-green-600 font-semibold">
                        Discount: -{formatCurrency(currentInvoice.totalDiscount)}
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div className="space-y-6 py-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div 
                  className={`p-4 border-2 rounded-xl cursor-pointer transition-all duration-200 ${
                    selectedPaymentMethod === 'cash'
                      ? 'border-[#2B7A78] bg-[#DFF5F4] shadow-lg scale-[1.02]'
                      : 'border-gray-200 hover:border-[#2B7A78] hover:shadow-md bg-white'
                  }`}
                  onClick={() => setSelectedPaymentMethod('cash')}
                >
                  <div className="flex items-center space-x-3">
                    <div className="flex items-center justify-center w-12 h-12 bg-[#2B7A78] rounded-xl">
                      <Banknote className="h-6 w-6 text-white" />
                    </div>
                    <div>
                      <h4 className="font-semibold text-[#1E293B]">Cash</h4>
                      <p className="text-sm text-gray-600">Pay with cash</p>
                    </div>
                  </div>
                  {selectedPaymentMethod === 'cash' && (
                    <div className="mt-3 flex items-center space-x-2 text-[#2B7A78]">
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h4 className="font-semibold text-[#1E293B]">Card</h4>
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h4 className="font-semibold text-[#1E293B]">Digital</h4>
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h4 className="font-semibold text-[#1E293B]">Credit</h4>
            ...
            {selectedPaymentMethod === 'cash' && (
              <div className="space-y-4 p-4 bg-[#DFF5F4] border border-[#2B7A78] rounded-lg">
                <h4 className="font-semibold text-[#1E293B] flex items-center space-x-2">
                  <Banknote className="h-4 w-4 text-[#2B7A78]" />
                  <span>Cash Payment Details</span>
                </h4>
                <div>
                  <Label className="text-[#1E293B]">Amount Received (AED)</Label>
                  <Input
                    type="number"
                  ...
                  {parseFloat(receivedAmount) > currentInvoice.total && (
                    <div className="mt-2 p-3 bg-green-50 border border-green-200 rounded">
                      <p className="text-sm font-semibold text-green-700">
                        Change to Return: {formatCurrency(parseFloat(receivedAmount) - currentInvoice.total)}
                      </p>
                    </div>
                  )}
                  {receivedAmount && parseFloat(receivedAmount) < currentInvoice.total && (
          ...
          <DialogFooter className="flex justify-end space-x-3 pt-4 border-t">
            ...
            <Button
              ...
              disabled={!selectedPaymentMethod || (selectedPaymentMethod === 'cash' && (!receivedAmount || parseFloat(receivedAmount) < currentInvoice.total))}
              className="bg-[#2B7A78] hover:bg-[#236862] text-white"
            >
              <CheckCircle className="h-4 w-4 mr-2" />
              Complete Payment
